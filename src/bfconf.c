/*
 * (c) Copyright 2001 - 2006, 2013, 2025 -- Anders Torger
 *
...
static bool has_defaults = false;
...
field_repeat_test(uint32_t *bitset,
                  int bit)
{
    if (bit32_isset(bitset, bit)) {
        parse_error("Field is already set.\n");
    }
    bit32_set(bitset, bit);
}
                    ...
                    bool allow_auto)
{
    ...
    if (ascii_strcasecmp(s, "S8") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S8;
        sf->bytes = 1;
        sf->sbytes = 1;
        little_endian = false;
    } else if (ascii_strcasecmp(s, "S16_LE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S16_LE;
        sf->bytes = 2;
        sf->sbytes = 2;
    } else if (ascii_strcasecmp(s, "S16_BE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S16_BE;
        sf->bytes = 2;
        sf->sbytes = 2;
        little_endian = false;
    } else if (ascii_strcasecmp(s, "S16_NE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S16_NE;
        sf->bytes = 2;
        sf->sbytes = 2;
        native_endian = true;
    } else if (ascii_strcasecmp(s, "S24_LE") == 0 || ascii_strcasecmp(s, "S24_3LE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S24_LE;
        sf->bytes = 3;
        sf->sbytes = 3;
    } else if (ascii_strcasecmp(s, "S24_BE") == 0 || ascii_strcasecmp(s, "S24_3BE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S24_BE;
        sf->bytes = 3;
        sf->sbytes = 3;
        little_endian = false;
    } else if (ascii_strcasecmp(s, "S24_NE") == 0 || ascii_strcasecmp(s, "S24_3NE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S24_NE;
        sf->bytes = 3;
        sf->sbytes = 3;
        native_endian = true;
    } else if (ascii_strcasecmp(s, "S24_4LE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S24_4LE;
        sf->bytes = 4;
        sf->sbytes = 3;
    } else if (ascii_strcasecmp(s, "S24_4BE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S24_4BE;
        sf->bytes = 4;
        sf->sbytes = 3;
        little_endian = false;
    } else if (ascii_strcasecmp(s, "S24_4NE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S24_4NE;
        sf->bytes = 4;
        sf->sbytes = 3;
        native_endian = true;
    } else if (ascii_strcasecmp(s, "S32_LE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S32_LE;
        sf->bytes = 4;
        sf->sbytes = 4;
    } else if (ascii_strcasecmp(s, "S32_BE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S32_BE;
        sf->bytes = 4;
        sf->sbytes = 4;
        little_endian = false;
    } else if (ascii_strcasecmp(s, "S32_NE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_S32_NE;
        sf->bytes = 4;
        sf->sbytes = 4;
        native_endian = true;
    } else if (ascii_strcasecmp(s, "FLOAT_LE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_FLOAT_LE;
        sf->bytes = 4;
        sf->sbytes = 4;
        sf->isfloat = true;
    } else if (ascii_strcasecmp(s, "FLOAT_BE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_FLOAT_BE;
        sf->bytes = 4;
        sf->sbytes = 4;
        sf->isfloat = true;
        little_endian = false;
    } else if (ascii_strcasecmp(s, "FLOAT_NE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_FLOAT_NE;
        sf->bytes = 4;
        sf->sbytes = 4;
        native_endian = true;
    } else if (ascii_strcasecmp(s, "FLOAT64_LE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_FLOAT64_LE;
        sf->bytes = 8;
        sf->sbytes = 8;
        sf->isfloat = true;
    } else if (ascii_strcasecmp(s, "FLOAT64_BE") == 0) {
        sf->format = BF_SAMPLE_FORMAT_FLOAT64_BE;
        sf->bytes = 8;
        sf->sbytes = 8;
        sf->isfloat = true;
        little_endian = false;
    } else if (ascii_strcasecmp(s, "FLOAT64_NE") == 0) {
...
static struct coeff *
parse_coeff(bool parse_default,
            int intname)
{
    struct coeff *coeff;
    uint32_t bitset = 0;
    ...
    do {
        ...
        case FIELD:
            if (strcmp(yylval.field, "format") == 0) {
                ...
                if (ascii_strcasecmp(yylval.string, "text") == 0) {
                    coeff->format = COEFF_FORMAT_TEXT;
                } else if (ascii_strcasecmp(yylval.string, "processed") == 0) {
...
parse_filter(bool parse_default,
             int intname)
{
    ...
    if (!parse_default) {
        if (!bit32_isset(&bitset, 5) && !bit32_isset(&bitset, 3)) {
            parse_error("no outputs for filter.\n");
        }
        if (!bit32_isset(&bitset, 4) && !bit32_isset(&bitset, 2)) {
...
static struct iodev *
parse_iodev(bool parse_default,
            int io,
            ...
            if (strcmp(yylval.field, "device") == 0) {
                ...
                case SLASH:
                    ...
                    for (n = 0; n < iodev->ch.used_channels; n++) {
                        ...
                        if (bit32_isset(inuse, iodev->ch.channel_selection[n])) {
                            parse_error("duplicate channel selection.\n");
                        }
                        bit32_set(inuse, iodev->ch.channel_selection[n]);
                    }
                    break;
                case EOS:
              ...
              int offset)
{
    ...
    if (n == n_shmids) {
        if ((buf = shmalloc_attach(shmid)) == NULL) {
            exit(BF_EXIT_OTHER);
        }
        if (n_shmids == cap) {
            cap += 16;
    ...
    if (((uintptr_t)p & (ALIGNMENT - 1)) != 0) {
        fprintf(stderr, "Shared memory pointer with id %d and offset %d is not aligned at a %d byte boundary.\n",
                shmid, offset, ALIGNMENT);
        exit(BF_EXIT_OTHER);
    }
        ...
        do {
            ...
            for (i = 0; i < bfconf->n_filters; i++) {
                ...
                for (j = 0; j < bfconf->filters[i].n_channels[OUT]; j++) {
                    bit32_set(used_channels, bfconf->filters[i].channels[OUT][j]);
                }
            }
            ...
            bool nodefault)
{
    ...
    FOR_IN_AND_OUT {
        ...
        bfconf->mute[IO] = emalloc(BF_MAXCHANNELS * sizeof(bool));
        memset(bfconf->mute[IO], 0, BF_MAXCHANNELS * sizeof(bool));
    }
    bfconf->sdf_length = -1;
    bfconf->quiet = quiet;
    ...
    if (filename != NULL && ascii_strcasecmp(filename, "stdin") == 0) {
    ...
    for (n = 0; n < bfconf->n_filters; n++) {
        ...
        if (pfilters[n]->coeff_name[0] == '\0') {
            if (pfilters[n]->fctrl.coeff >= bfconf->n_coeffs) {
                fprintf(stderr, "Coeff index %d in filter %d/\"%s\" is out of range.\n",
                        pfilters[n]->fctrl.coeff, n, pfilters[n]->filter.name);
                exit(BF_EXIT_INVALID_CONFIG);
            }
        } else {
            ...
            if (pfilters[n]->fctrl.coeff == -1) {
                fprintf(stderr, "Coeff with name \"%s\" (in filter %d/\"%s\") does not exist.\n",
                        pfilters[n]->coeff_name, n, pfilters[n]->filter.name);
                exit(BF_EXIT_INVALID_CONFIG);
            }
        }
        ...
        if (pfilters[n]->process == -1) {
            ...
            bit32_set(used_processes, pfilters[n]->process);
            if (n > 0 && load_balance) {
                fprintf(stderr, "Cannot mix manual process settings with automatic.\n");
                exit(BF_EXIT_INVALID_CONFIG);
            }
        ...
        FOR_IN_AND_OUT {
            ...
            for (j = 0; j < pfilters[n]->filter.n_channels[IO]; j++) {
                ...
                bit32_set(used_channels[IO], pfilters[n]->filter.channels[IO][j]);
                if (bit32_isset(local_used_channels, pfilters[n]->filter.channels[IO][j])) {
                    fprintf(stderr, "Duplicate channels in filter %d/\"%s\".\n", n, pfilters[n]->filter.name);
                    exit(BF_EXIT_INVALID_CONFIG);
                }
                bit32_set(local_used_channels, pfilters[n]->filter.channels[IO][j]);
            }
            ...
            for (j = 0; j < pfilters[n]->filter.n_filters[IO]; j++) {
                ...
                bit32_set(used_filters, pfilters[n]->filter.filters[IO][j]);
            }
        }
    ...
    FOR_IN_AND_OUT {
        for (n = 0; n < bfconf->n_channels[IO]; n++) {
            if (!bit32_isset(used_channels[IO], n)) {
    ...
    for (n = 0; n <= largest_process; n++) {
        if (!bit32_isset(used_processes, n)) {
...
"Input from filter %d/\"%s\" in filter %d/\"%s\" must exist\n\
  as output in the source filter %d/\"%s\".\n",
                        k, bfconf->filters[k].name, n, bfconf->filters[n].name, k, bfconf->filters[k].name);
                exit(BF_EXIT_INVALID_CONFIG);
            }
        }
    }
...
/*    if (convolver_init != NULL) {*/
        ...
        if (!convolver_init(convolver_config, bfconf->filter_length, bfconf->realsize)) {
...
/*    }*/
    ...
    if (bfconf->sdf_length < 0) {
        ...
        /* note: this check depends on the internals of delay_subsample_init() */
        fprintf(stderr, "The filter_length must be larger than 2 x sdf_length + 1.\n");
        exit(BF_EXIT_INVALID_CONFIG);
    }
    ...
    /* shorten mute array */
    FOR_IN_AND_OUT {
        bfconf->mute[IO] = erealloc(bfconf->mute[IO], bfconf->n_channels[IO] * sizeof(bool));
        bfconf->delay[IO] = erealloc(bfconf->delay[IO], bfconf->n_channels[IO] * sizeof(int));
    }

    /* derive filter_process from filters */
    bfconf->n_processes = largest_process + 1;
    bfconf->fproc = emalloc(bfconf->n_processes * sizeof(struct filter_process));
    memset(bfconf->fproc, 0, bfconf->n_processes * sizeof(struct filter_process));
    for (n = k = 0; n < bfconf->n_processes; n++) {
            ...
            if (pfilters[i]->process == n) {
                ...
                FOR_IN_AND_OUT {
                    for (j = 0; j < pfilters[i]->filter.n_channels[IO]; j++) {
                        if (!bit32_isset(used_channels[IO], pfilters[i]->filter.channels[IO][j])) {
                            channels[IO][n_channels[IO]] = pfilters[i]->filter.channels[IO][j];
                            n_channels[IO]++;
                        }
                        bit32_set(used_channels[IO], pfilters[i]->filter.channels[IO][j]);
                    }
                }
            }
        }
        ...
        FOR_IN_AND_OUT {
            ...
            bfconf->fproc[n].unique_channels[IO] = emalloc(n_channels[IO] * sizeof(int));
            memcpy(bfconf->fproc[n].unique_channels[IO], channels[IO], n_channels[IO] * sizeof(int));
        }

        bfconf->fproc[n].filters = emalloc(bfconf->fproc[n].n_filters * sizeof(struct bffilter));
        memcpy(bfconf->fproc[n].filters, filters, bfconf->fproc[n].n_filters * sizeof(struct bffilter));
    }
    ...
    for (n = 0; n < bfconf->n_processes; n++) {
        ...
        for (i = 0; i < bfconf->fproc[n].n_filters; i++) {
            for (j = 0; j < bfconf->fproc[n].filters[i].n_channels[OUT]; j++) {
                bit32_set(used_channels[OUT], bfconf->fproc[n].filters[i].channels[OUT][j]);
            }
            for (j = 0; j < bfconf->fproc[n].filters[i].n_filters[OUT]; j++) {
                bit32_set(used_filters, bfconf->fproc[n].filters[i].filters[OUT][j]);
            }
        }
        for (k = 0; k < bfconf->n_processes; k++) {
            if (k != n) {
                for (i = 0; i < bfconf->fproc[k].n_filters; i++) {
                    ...
                    {
                        if (bit32_isset(used_channels[OUT], bfconf->
    ...
    for (n = 0; n < bfconf->n_processes; n++) {
        ...
        while (j < bfconf->fproc[n].n_filters) {
            for (i = 0; i < bfconf->fproc[n].n_filters; i++) {
                if (bit32_isset(used_filters, bfconf->fproc[n].filters[i].intname)) {
                    continue;
                }
                for (k = 0;
                     ...
                     k++)
                {
                    if (!bit32_isset(used_filters, bfconf->fproc[n].filters[i].filters[IN][k])) {
                        k = -1;
                        break;
                    }
                }
                if (k != -1) {
                    ...
                    bit32_set(used_filters, filters[j].intname);
                    j++;
                }
            }
        }
    ...
    for (n = i = 0; n < bfconf->n_logicmods; n++) {
        ...
        if (bfconf->logicmods[n].preinit(&version_major,
                                         ...
                                         bfconf->coeffs,
                                         bfconf->n_channels,
                                         (const struct bfchannel **)bfconf->channels,
                                         bfconf->n_filters,
                                         bfconf->filters,
        ...
        if (version_major != BF_VERSION_MAJOR) {
            fprintf(stderr, "Logic module \"%s\" reported an incompatible major version (got %d, expected %d).\n",
    ...
    FOR_IN_AND_OUT {
        for (n = 0; n < bfconf->n_subdevs[IO]; n++) {
            for (i = 0; i < bfconf->n_iomods; i++) {
                if (strcmp(bfconf->ionames[i], iodevs[IO][n]->device_name) == 0) {
                    break;
                }
            }
            ...
            load_bfio_module(&bfconf->iomods[bfconf->n_iomods], iodevs[IO][n]->device_name);
            if (bfconf->iomods[bfconf->n_iomods].iscallback) {
                bfconf->callback_io = true;
            } else {
                bfconf->blocking_io = true;
            }
            bfconf->ionames[bfconf->n_iomods] = estrdup(iodevs[IO][n]->device_name);
        ...
        for (n = 0; n < bfconf->n_subdevs[IO]; n++) {
            ...
            bfconf->subdevs[IO][n].params =
                bfconf->iomods[i].preinit(&version_major,
                                          ...
                                          (int)bfconf->debug);
            bfconf->subdevs[IO][n].uses_clock = !!uses_clock;
            if (default_iodev[IO] == NULL ||
            ...
            {
                fprintf(stderr, "Module \"%s\" lacks the synch_start/stop functions.\n", bfconf->ionames[i]);
                exit(BF_EXIT_INVALID_CONFIG);
            }
            if (!bfconf->subdevs[IO][n].uses_clock &&
                ...
                !bfconf->iomods[i].iscallback)
            {
                fprintf(stderr, "Module \"%s\" lacks the start/stop functions.\n", bfconf->ionames[i]);
                exit(BF_EXIT_INVALID_CONFIG);
            }
            ...
            if (iodevs[IO][n]->auto_format) {
                if (bf_sampleformat_size(iodevs[IO][n]->ch.sf.format) <= 0) {
                    fprintf(stderr, "Module \"%s\" could not decide sample format.\n", bfconf->ionames[i]);
                    exit(BF_EXIT_OTHER);
                }
                parse_sample_format(&iodevs[IO][n]->ch.sf, bf_strsampleformat(iodevs[IO][n]->ch.sf.format), false);
            }
    ...
    for (n = j = 0; n < bfconf->n_subdevs[OUT]; n++) {
        ...
        if (iodevs[OUT][n]->ch.sf.isfloat) {
            if (!iodevs[OUT][n]->auto_format) {
                pinfo("Warning: cannot dither floating point format (outputs %d - %d).\n",
                      ...
                      iodevs[OUT][n]->channel_intname[iodevs[OUT][n]->virtual_channels - 1]);
            }
            continue;
        }
        ...
        for (i = 0; i < iodevs[OUT][n]->ch.used_channels; i++) {
            bit32_set(apply_dither, iodevs[OUT][n]->ch.channel_name[i]);
            j++;
        }
    }
...
External scheduling belonging to module \"%s\" must be realtime.\n",
                ...
                if (bfconf->realtime_priority) {
                    if (max_prio < bfconf->subdevs[IO][n].sched_param.sched_priority) {
                        max_prio = bfconf->subdevs[IO][n].sched_param.sched_priority;
                    }
                    if (bfconf->subdevs[IO][n].sched_param.sched_priority < min_prio || min_prio == -1) {
                        min_prio = bfconf->subdevs[IO][n].sched_param.sched_priority;
                    }
                    if (bfconf->realtime_midprio > bfconf->subdevs[IO][n].sched_param.sched_priority) {
                        fprintf(stderr, "\
External scheduling priority for module \"%s\" is lower than %d.\n",
    ...
    if (tv2.tv_sec == 0 && tv2.tv_usec < 100000) {
        compat_usleep(100000 - tv2.tv_usec);
        gettimeofday(&tv2, NULL);
        timestamp(&t2);
        timersub(&tv2, &tv1, &tv2);
    }
